...
template<class Record,int order>
Error_code B_tree<Record,order>::recursive_search_tree(B_node<Record,order>*current, Record&data)
{
	Error_code result=not_present;
	int position; 
	if(current!=0){
		result=search_node(current,data,position);
		if(result==not_present)
		result=recursive_search_tree(current->data[position],data)
			target=current->data[position];
	}
	return result;
}
template<class Record,int order>
Error_code B_tree<Record,order>::search_node(B_node<Record,order>*current,const Record&data,int position)
{
	position=0;
	while(position<current->count&&target>current->data[position])
		position++;
	if(position<current&&target==current->data[position])
		return success;
	else
		return not_present;
}
...
Error_code B_tree<Record,order>::insert(const Record&new_entry)
{
	...
	Error_code result=push_dowm(root,new_entry,median,right_branch);
	if(result==overflow){
...
void B_tree<Entry,order>::split_node(B_node<Entry,order>*current,const Entry&extra_entry,B_node<Entry,order>*extra_branch,int position,B_node<Entry,order>*&right_half, Entry&median)
{
	...
	if(position<=mid){
		...
		median=current->data[currrent->count-1];
...
void B_tree<Record,order>::copy_in_predecessor(B_node<Record,order>*current , int position)
{
	...
	while(leaf->branch=leaf->count]!=0)
		leaf=leaf->branch[leaf->count];
		current->data[position]=leaf->data[leaf->count-1];
}
template<class Record,int order>
void B_tree<Record,order>::restore(B_tree<Record,order>*current,int position)
{
	if(position==current->count)
		if(current->branch[position-1]->count>(order-1)/2)
			...
			if(current->branch[1]->count>(order-1)/2)
				...
				if(current[position-1]>(order-1)/2)
					move_right(current,position-1);
				else if(current[position+1]>(order-1)/2)
					move_left(current,position+1);
				else
					combine(current,position);
}
template<class Record,int order>
void B_tree<Record,int oeder>::move_left(B_node<Record,order>*current,int position)
{
...
void B_tree<Record,order>::move_right(B_node<Record,order>*current,int position)
{
	...
	for(int i=right_branch->count-1;i>=0;i++)
	{
		right_branch->data[i+1]=right_branch->data[i];
		right_branch->branch[i+1]=right_branch->beanch[i];
	}
	right_branch++;
	right_data[0]=current->data[position];
	right_branch[0]=left_branch->branch[left_branch->count+1];
	current->data[position]=left_branch[left_branch->count-1];
	left_branch->count--;
}
template<class Record,int order>
void B_tree<Record,order>::combine(B_tree<Record,order>*current,int position)
{
	int i;
	B_node<Record,order>*left_branch=current->branch[position-1];
	*right_branch=current->branch[position];
	left_branch->data[left_branch->count]=current->data[position-1];
	left_branch->count++;
	for(i=0;i<right_count->count;i++)
	{
		left_branch->data[left_branch->count]=right_branch->data[i];
		left_branch->branch[left_branch_count++]=right_branch->data[i+1];
	}
	current->count--;
