Error_code{success,not_present}
...
class B_tree{
	...
	private:
		...
		Error_code push_down(B_node<Record,order>*current,const Record&new_entry,Record&median,B_node<Record,order>*right_banch);
		Error_code push_in(B_node<Record,data>*current, const Record&entry,B_node<Record,order>*right_branch,int position);
		void split_node(B_tree<Record,order>*current,const Record&extra_entry,B_node<Record,order>*extra_banch,int position,B_node<Record,order>*&right_half,Record &median);
		...
		void combine(B_node<Record,oeder>*current,int position);
};
